$(document).ready(function(e) {
    ...
    function autoHeight(){
		...
		$(".watch-embed-fr.haschat").css("height",windowHeight-240);
		...
		$('.box').css('height',Math.min(videoHeight,(windowHeight-160)));/////box height
		$(".watch-fr .memberlist-box").css("height",windowHeight-56);
		$(".watch-embed-fr.haschat .memberlist-box").css("height",windowHeight-296);
		if($(".watch-embed-fr.haschat").hasClass("hasdoc")){
			$(".watch-fr .memberlist-box").css("height",windowHeight-56);
		}
		$(".watch-fr .chatlist-box,.watch-embed-fr.haschat.hasdoc .chatlist-box").css("height",windowHeight-170);
		$(".watch-embed-fr .chatlist-box").css("height",windowHeight-120);
		$(".watch-embed-fr.haschat .chatlist-box").css("height",windowHeight-360);
		if($(".watch-embed-fr.haschat").hasClass("hasdoc")){
			$(".watch-embed-fr .chatlist-box").css("height",windowHeight-120);
		}
		$(".watch-fr .chat-area").css("height",windowHeight-170);
		$(".watch-embed-fr.haschat .chat-area").css("height",windowHeight-410);
		if($(".watch-embed-fr.haschat").hasClass("hasdoc")){
			$(".watch-embed-fr .chat-area").css("height",windowHeight-170);
		}
    ...
    $(".arrow-right").click(function(){
        ...
        if(target.css("display")=="block"){
            ...
            target.show();
            $(".watch-box").css("margin-right",290); 
            $(this).css("right",260).removeClass("left").html("&gt");
        }  
    });
	$(".chatlist").on('click','.follow-someone',function(){
		vhallApp.followSomeone($(this));
	});
	...
	$("#to-login").click(function(){
		...
		if(completed){
			var pars={'account':$("#username").val(),'password':$("#pwd").val()/*,'captcha':$(".v-code").val()*/};
			...
			if(typeof toLoginUrl!="undefined"){
				...
				loginUrl=domain+'/auth/ajaxlogin';
			}
			$.ajax({
				url:loginUrl,
				type:'post',
	...
	$("#tosearch").keydown(function(e){ 
		...
		if(curKey == 13){ 
			...
			return false;   
				...
				window.location.href=searchPageUrl+'/'+$(this).val();  
			}  
			return false; 
		} 
	});
	if($(".watch-embed-box").length==0){
		$("body").css({"min-width":1000,"min-height":420});	
	}
	if($(".watch-fr").hasClass("chatonly")){
		$("body").css({"min-width":"100%","min-height":"100%"});		
	}
