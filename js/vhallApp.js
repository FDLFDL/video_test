...
var vhallApp={
	...
	showMsg:function(msg,warning){
		if($("#showMsg").length==0){
			var msg='<div id="showMsg"><span class="icon"></span><p class="msg">'+msg+'</p></div>';	
			$("body").append($(msg));
	...
	showGuide:function(target,img,disX,disY,guide){
		if($(".guide-mask").length==0){
			...
			var guideBox='<div class="'+guide+'"><img src='+img+' alt=""><a href="javascript:void(0);" class="'+btnClass+'"></a></div>';
			$("body").append($(guideMask));
			$("body").append($(guideBox));
		}else{
			if(guide=="guide-box-set2"){
				$(".guide-box-set1").hide();
				var guideBox='<div class="'+guide+'"><img src='+img+' alt=""><a href="javascript:void(0);" class="next"></a></div>';
				$("body").append($(guideBox));
				
			}else if(guide=="guide-box-set3"||guide=="guide-box-edit2"){
				$(".guide-box-set2").hide();
				$(".guide-box-edit1").hide();
				var guideBox='<div class="'+guide+'"><img src='+img+' alt=""><a href="javascript:void(0);" class="close"></a></div>';
				$("body").append($(guideBox));		
			}
		}
		$("."+guide).css({"position":"absolute","left":($(target).offset().left+disX),"top":$(target).offset().top+disY});
	...
	floatCard:function(parent,sclass,target){
		...
		parent.on('mouseenter',sclass,function(e){
			...
			if(target.is("#shareout-div")){
				...
				target.css({left:(xDis-85),top:(yDis+20)}).show();	
			}else{
				target.css({left:(xDis+20),top:(yDis+30)}).show();
	...
	sendMsg:function(url){
		...
		$("#sendMsg .btn").click(function(){
			$.ajax({
				...
				type:'post',
				data:{'user_id':sendMsgId, 'content':$("#msg-con").val()},
				beforeSend: function() {
